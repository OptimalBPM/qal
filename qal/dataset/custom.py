...
class CustomDataset(object):
        ...
        if self._log_level >= DATASET_LOGLEVEL_DETAIL:
            ...
            for _field_idx in range(len(self.field_names)):
                if _old_row[_field_idx] != _new_row[_field_idx]:
                    _field_diffs.append(
                        self.field_names[_field_idx] + " : " + quote(str(_old_row[_field_idx])) + " =>" + quote(
                            str(_new_row[_field_idx])))

            self._log.append(self.__class__.__name__ + ".update;" + quote(str(_row_key)) + ";" + ";" + "|".join(
                _field_diffs) + empty_if_none(";" + str(_comment), _comment))

    def log_insert(self, _row_key, _row_data, _comment=None):
        if self._log_level >= DATASET_LOGLEVEL_DETAIL:
            self._log.append(self.__class__.__name__ + ".insert;" + quote(str(_row_key)) + ";" + quote(
                str(_row_data)) + empty_if_none(";" + str(_comment), _comment))

    def log_delete(self, _row_key, _row_data, _comment=None):
        if self._log_level >= DATASET_LOGLEVEL_DETAIL:
            self._log.append(self.__class__.__name__ + ".delete;" + quote(str(_row_key)) + ";" + quote(
                str(_row_data)) + empty_if_none(";" + str(_comment), _comment))

    def log_save(self, _filename, _comment=None):
        if self._log_level >= DATASET_LOGLEVEL_LOW:
            self._log.append(self.__class__.__name__ + ".save;" + quote(str(_filename)) + ";" + str(
                datetime.now().isoformat()) + empty_if_none(";" + str(_comment), _comment))

    def log_load(self, _filename, _comment=None):
        if self._log_level >= DATASET_LOGLEVEL_LOW:
            self._log.append(self.__class__.__name__ + ".load;" + quote(str(_filename)) + ";" + str(
                datetime.now().isoformat()) + empty_if_none(";" + str(_comment), _comment))
    ...
    def _structure_apply_merge(self, _insert, _update, _delete, _sorted_dest, _commit=True):
        ...
        if len(self._structure_key_fields) >= len(self.field_names):
            raise Exception(
                "Error in RDBMSDataset.structure_init: Updates cannot be made when there are no non-key fields.")
    ...
    def apply_new_data(self, _new_data_table, _key_fields, _insert=None, _update=None, _delete=None, _commit=True):
        ...
        :parameter 2D-list _new_data_table: A two-dimensional list contains the data.\
         Must match the existing column-wise.
        :parameter list _key_fields: An array with the indices of the fields that should be used to match source \
         rows to destination rows.
        ...
        # Note: in RDBMSDataset, this currently means writing to the underlying database,
        # since there is no in-memory structure.
    ...
    def as_sql(self, _db_type):
        ...
        for _row in self.data_table:
            ...
            for _col_idx in range(len(_row)):
                ...
                if _add_field_names:
                    _curr_row.append(
                        _str_col + ' AS ' + db_specific_object_reference(self.field_names[_col_idx], _db_type))
                else:
