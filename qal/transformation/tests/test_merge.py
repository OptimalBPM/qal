...
class MergeTest(unittest.TestCase):
    ...
    def test_1_Merge_files(self):
        ...
        self.assertEqual(etree.tostring(_merge.as_xml_node()),
            ...
            "Input/output XML does not match")
    ...
    def test_2_Merge_tables(self):
        ...
        _resources_node = self._parse_xml(os.path.join(Test_Resource_Dir, "test_merge_two_tables.xml")).find(
            "resources")
        ...
        self.assertEqual(etree.tostring(_merge.as_xml_node()), etree.tostring(_merge_xml),
            "Input/output XML does not match")
    ...
    def test_3_Merge_to_nonexisting(self):
        ...
        _resources_node = self._parse_xml(
            os.path.join(Test_Resource_Dir, "test_merge_no_keys_to_nonexisting.xml")).find("resources")
        _resources = Resources(_resources_node=_resources_node)
        print("merge_test.test_Merge_tables: Staging source")
        _source_data = [[1, 'source', datetime.datetime(2001, 1, 1, 0, 0)],
                        [2, 'source', datetime.datetime(2001, 1, 2, 0, 0)],
